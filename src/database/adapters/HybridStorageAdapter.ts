...
/** Render a rejected id in an error message without dumping an object graph. */
...
/**
 * Reject a workspace id that is not a usable string, before it reaches a
 * repository.
 *
 * Every workspace-scoped write derives its JSONL stream from the id via
 * `workspaces/ws_${workspaceId}.jsonl`. Template interpolation happily
 * stringifies anything, so passing an object created a real, permanent
 * `ws_[object Object]` stream on disk — directory, shard and all — with the
 * events inside unreachable from any workspace that exists. The adapter is the
 * boundary where callers hand over untyped data (tool params, older callers,
 * plain JS), so the check belongs here rather than in the repositories.
 */
export function assertValidWorkspaceId(value: unknown, method: string): asserts value is string {
  if (typeof value !== 'string' || value.trim().length === 0) {
    throw new Error(
      `${method}: workspaceId must be a non-empty string, received ${describeInvalidId(value)}`
    );
  }
}
...
export class HybridStorageAdapter implements IStorageAdapter {
  ...
  createWorkspace = async (workspace: Omit<WorkspaceMetadata, 'id'> & { id?: string }): Promise<string> => {
    // Only when supplied — an omitted id is generated by the repository.
    if (workspace?.id !== undefined) {
      assertValidWorkspaceId(workspace.id, 'createWorkspace');
    }
    await this.ensureInitialized();
  ...
  updateWorkspace = async (id: string, updates: Partial<WorkspaceMetadata>): Promise<void> => {
    assertValidWorkspaceId(id, 'updateWorkspace');
    await this.ensureInitialized();
    return this.workspaceRepo.update(id, updates);
  };

  deleteWorkspace = async (id: string): Promise<void> => {
    assertValidWorkspaceId(id, 'deleteWorkspace');
    await this.ensureInitialized();
  ...
  createSession = async (workspaceId: string, session: Omit<SessionMetadata, 'id' | 'workspaceId'> & { id?: string }): Promise<string> => {
    assertValidWorkspaceId(workspaceId, 'createSession');
    await this.ensureInitialized();
  ...
  updateSession = async (workspaceId: string, sessionId: string, updates: Partial<SessionMetadata>): Promise<void> => {
    assertValidWorkspaceId(workspaceId, 'updateSession');
    await this.ensureInitialized();
  ...
  moveSessionToWorkspace = async (sessionId: string, workspaceId: string): Promise<void> => {
    assertValidWorkspaceId(workspaceId, 'moveSessionToWorkspace');
    await this.ensureInitialized();
  ...
  saveState = async (
    ...
    assertValidWorkspaceId(workspaceId, 'saveState');
    await this.ensureInitialized();
  ...
  addTrace = async (
    ...
    assertValidWorkspaceId(workspaceId, 'addTrace');
    await this.ensureInitialized();
