...
- Context/session helpers: `contextUtils.ts`, `sessionUtils.ts`, `workspaceUtils.ts`, `toolContextUtils.ts` (tool-call context normalization)
...
- Keep tool-call boundaries using `normalizeToolContext(...)` so session/workspace handling stays consistent.
