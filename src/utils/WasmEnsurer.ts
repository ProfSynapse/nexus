...
export class WasmEnsurer {
    ...
    private async downloadWasm(wasmPath: string): Promise<boolean> {
        const notice = new Notice('Downloading the sqlite3 wasm file. This only happens once.', 0);
        ...
        try {
            for (const url of WasmEnsurer.WASM_URLS) {
                try {
                    ...
                    notice.hide();
                    new Notice('Sqlite3 wasm file downloaded successfully.', 3000);
                    return true;
            ...
            // All URLs failed
            notice.hide();
            new Notice('Failed to download the sqlite3 wasm file. Check your internet connection and restart the app.', 10000);
            return false;

        } catch (error) {
            notice.hide();
            console.error('[WasmEnsurer] Download failed:', error);
            new Notice('Failed to download the sqlite3 wasm file. Some features may not work.', 10000);
            return false;
        }
    }
