 ...
 * Generated: 2026-07-19T10:28:45.173Z
 */

/** Combined content hash — used to detect and refresh a stale on-disk install. */
export const NEXUS_CLI_ASSETS_HASH = "50a530db1cdcf19b";
...
function listVaultSockets() {
  try {
    ...
    return (0, import_node_fs2.readdirSync)(UNIX_SOCK_DIR).filter((f) => f.startsWith(NAME_PREFIX) && f.endsWith(UNIX_SUFFIX)).map((f) => ({ name: f.slice(NAME_PREFIX.length, -UNIX_SUFFIX.length), path: \`\${UNIX_SOCK_DIR}/\${f}\` }));
  } catch {
    return [];
  }
}
