...
- 2026-09-22 | Adding GPT-6 Sol and Luna: the OpenAI and Codex adapters dropped `temperature`/`top_p` for `model !== 'gpt-6-astra'` only, and a live probe showed Sol and Luna reject both with a 400 too. Step 5's routing grep only matched `includes`/`startsWith`, so it could not have found an exact-id guard, and it used `rg`, which is not installed on every dev machine. | Widened the step-5 pattern to exact-id equality and regex tests and switched it to `grep -E`. At the user's request the guard became a typed `ModelSpec.supportsSamplingParams` flag rather than a family predicate. Probing every registry model live showed 11 OpenAI and 6 Anthropic models reject sampling, including the OpenAI default `gpt-5.6-sol`, and none of them had been caught because the smoke lane never sent a temperature. The smoke lane now sends one by default. | Documented the field in `references/registry-anatomy.md` and in step 5 of `protocols/add-model.md`. | Files:   `protocols/add-model.md`, `references/registry-anatomy.md`.

- 2026-09-25 | A working-tree entry for OpenRouter's
  `~deepseek/deepseek-flash-latest` recorded $0.10/$0.50 per 1M from a
  2026-09-23 snapshot of the listing; re-reading the listing at verification
  time on 2026-09-25 showed $0.04/$0.49. No skill gap: the protocol already
  makes the listing the authority, so the fix was the protocol working. Logged
  as evidence that a snapshot read in an earlier session must be re-read when
  the entry is actually written, not reused. | No skill-file change; corrected
  the registry entry and its unit expectation. | Files:
