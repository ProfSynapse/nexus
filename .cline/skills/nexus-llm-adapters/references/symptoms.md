...
| First tool call works, then the reply after the tool result is blank — every model on one provider | The adapter never reads `options.conversationHistory`, so the continuation request carries no tool calls or results and the model answers an empty conversation with an empty reply. Grep the adapter for `conversationHistory`; a request built from `prompt + systemPrompt` alone cannot continue a tool loop. (GroqAdapter shipped this way until 2026-08-27; the eval harness exposed it — 5% pass pre-fix, 97–100% post-fix.) | `chat-plumbing.md` |
