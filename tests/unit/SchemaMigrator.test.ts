...
describe('SchemaMigrator v11 -> v12 shard_cursors migration', () => {
  ...
  it('runs the v12 + v13 + v14 + v15 migrations when starting at v11', async () => {
...
describe('SchemaMigrator v12 -> v13 skills migration', () => {
  ...
  it('runs the v13 migration (and the later v14 + v15) when starting at v12', async () => {
    const db = new FakeDatabase();
    db.execResponders.push(
      { match: /sqlite_master.*schema_version/i, rows: [['schema_version']] },
...
describe('SchemaMigrator v13 -> v14 notes query index migration', () => {
  ...
  it('runs the v14 + v15 migrations when starting at v13', async () => {
    const db = new FakeDatabase();
    db.execResponders.push(
      { match: /sqlite_master.*schema_version/i, rows: [['schema_version']] },
...
describe('SchemaMigrator v14 -> v15 state archive flag migration', () => {
  it('includes a v15 migration that adds states.isArchived and its index', () => {
    const v15 = MIGRATIONS.find(m => m.version === 15);
    expect(v15).toBeDefined();
    expect(v15!.description.toLowerCase()).toContain('isarchived');
  ...
  it('adds the column WITHOUT a default so migrated rows stay unknown, not "not archived"', () => {
    ...
    const v15 = MIGRATIONS.find(m => m.version === 15)!;
    const alter = v15.sql.find(s => /ALTER TABLE states ADD COLUMN isArchived/i.test(s))!;
    expect(alter.toUpperCase()).not.toContain('DEFAULT');
  });

  it('uses additive-only DDL for v15 (ADD COLUMN / IF NOT EXISTS, no DROP or RENAME)', () => {
    const v15 = MIGRATIONS.find(m => m.version === 15)!;
  ...
  it('runs only the v15 migration when starting at v14', async () => {
    const db = new FakeDatabase();
    db.execResponders.push(
      { match: /sqlite_master.*schema_version/i, rows: [['schema_version']] },
