...
| **Web Tools** *(experimental, mostly desktop)* | open, capture-pdf, capture-png, capture-markdown, links | Open any webpage in a headless browser and capture it as a PDF, PNG, or clean Markdown (boilerplate stripped). Also extracts all links with their text and type. **capture-markdown works on mobile**: it fetches and extracts the page directly, with no browser involved. On desktop it falls back to the live browser view when a page turns out to be JavaScript-rendered or comes back empty — use `--transport fetch` or `--transport browser` to force one. The other four tools need a desktop browser and are not available on mobile. |
